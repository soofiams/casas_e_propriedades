...
if (!empty($_POST["apagar"]) && !empty($_POST["resposta"])) {
    ...
    idu_sql("DELETE FROM portefolio WHERE id = $id");
    header("Location: portefolio.php");

}

$portefolio_item = null;
if ($apagar) {
...
<body class="container-fluid">
    ...
    <main class="row">
        <?php if ($portefolio_item): ?>
            <p>Tem certeza de que deseja apagar o carrossel <strong><?= $portefolio_item["nome"] ?><?= $portefolio_item["localidade"] ?></strong>?</p>
            <form method="post">
                <input type="hidden" name="apagar" value="<?= $portefolio_item["id"] ?>">
                <input type="hidden" name="resposta" value="sim">
                <button type="submit" class="btn btn-danger">Sim, apagar</button>
                <a href="portefolio.php" class="btn btn-secondary">Cancelar</a>
            </form>
        <?php endif; ?>
    </main>
</body>
