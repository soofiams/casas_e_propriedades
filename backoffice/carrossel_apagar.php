...
if (!empty($_POST["apagar"]) && !empty($_POST["resposta"])) {
    ...
    idu_sql("DELETE FROM carrossel_header WHERE id = $id");
...
<body class="container-fluid">
    ...
    <main class="row">
        <?php if ($carrossel_item): ?>
            <p>Tem certeza de que deseja apagar o carrossel <strong><?= $carrossel_item["titulo"] ?></strong>?</p>
            <form method="post">
                <input type="hidden" name="apagar" value="<?= $carrossel_item["id"] ?>">
                <input type="hidden" name="resposta" value="sim">
                <button type="submit" class="btn btn-danger">Sim, apagar</button>
                <a href="carrossel.php" class="btn btn-secondary">Cancelar</a>
            </form>
        <?php endif; ?>
    </main>
</body>
