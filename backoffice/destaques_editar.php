...
$des = select_sql_unico("SELECT * FROM imovel WHERE id=$id");

$form = !empty($_POST["imagem"]) && isset($_POST["posicao"]) && isset($_POST["mostrar"]);
...
if ($form) {
  ...
  idu_sql("UPDATE immovel 
             SET imagens='$imagem', nome='$nome', tipo='$tipologia', 
    ...
    <div class="row">
      <div class="col-4">
        ...
        <form method="post" class="caixa form-login">
          ...
          <input type="text" value="<?= $des["imagens"] ?>" name="imagem" id="imagem" required readonly>
          <button type="button" class="ima-btn" onclick="abrir_popup('imagem')">&#128247</button>

          <label for="titulo">Título:</label>
          <input type="text" value="<?= $des["nome"] ?>" name="nome">

          <label for="subtitulo">Subtítulo:</label>
          <input type="text" value="<?= $des["tipologia"] ?>" name="tipologia">

          <label for="posicao">Posição:</label>
          <input type="number" value="<?= $des["posicao"] ?>" name="posicao" min="1" required>

          <label for="mostrar">Ativo:</label>
          <select name="ativo">
            <option value="1" <?= $des["mostrar"] == 1 ? "selected" : "" ?>>Sim</option>
            <option value="0" <?= $des["mostrar"] == 0 ? "selected" : "" ?>>Não</option>
          </select>

          <hr>
          <p>
            <a href="destaques.php"> <button type="button" class="sair-btn">Cancelar</button></a>
