...
$imovel = select_sql("SELECT * FROM imovel ORDER BY posicao ASC");
?>
...
<body class="container-fluid">
  ...
  <main class="container my-5">
    <h2>Tipos de Fogo <span class="desc">Listagem de todos os projetos</span></h2>
    <hr>
    <a href="tipos_novo.php">Novo</a>
    <hr>
    <table class="table table-bordered table-striped mt-3">
      <thead>
        <tr>
          <th>ID</th>
          <th>IMAGEM</th>
          <th>PROJETO EM COMERCIALIZAÇÃO</th>
          <th>OBSERVAÇÃO</th>
          <th>POSIÇÃO</th>
          <th>ATIVO</th>
          <th>AÇÕES</th>
      ...
      <tbody>
        <?php foreach ($imovel as $i): ?>
          <?php $imagens = json_decode($i["imagem"]) ?>
          <tr>
            <td><?= $i["id"] ?></td>
            ...
            <td><?= $i["tipologia"] ?></td>
            <td><?= $i["posicao"] ?></td>
            <td><?= $i["mostrar"] ? "Sim" : "Não" ?></td>
            <td>
              <a href="tipos_editar.php?id=<?= $i["id"] ?>">Editar</a>
              <a href="tipos_apagar.php?id=<?= $i["id"] ?>">Apagar</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
