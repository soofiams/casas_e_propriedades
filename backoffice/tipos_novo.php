...
$imoveis = get_todos_imoveis();

if (!empty($_POST)) {
  $imagem = $_POST["imagem"];
  $nome = $_POST["nome"];
  $tipologia = $_POST["tipologia"];
  $id_projeto = intval($_POST["id_projeto"]);
  $posicao = intval($_POST["posicao"]);
  $ativo = intval($_POST["mostrar"]);

  idu_sql("INSERT INTO imovel (imagem, nome, tipologia, id_projeto, posicao, mostrar) 
             VALUES ('$imagem', '$nome', '$tipologia', $id_projeto, $posicao, $ativo)");
...
<body class="container-fluid">
  ...
  <main>
    <div class="row">
      <div class="col-4">
        <br><br>
        <h2>Carrossel <span class="desc">Inserir novo Tipo Fogo.</span></h2>
        <hr>
        <a href="tipos.php">Fechar</a>

        <form action="" class="caixa form-login" method="post">
          <label for="imagens">Imagem: *</label>
          <input type="text" name="imagens" id="imagem" required readonly>
          ...
          <label for="subtitulo">tipologia: *</label>
          <input type="text" name="tipologia" placeholder="subtitulo" required>
          <label for="texto">ID produto: *</label>
          <input type="text" name="id_projeto" placeholder="Texto" required>
